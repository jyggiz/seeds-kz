...
module.exports = async function writeTermsOfUse(lang) {
  const singleType = await client.getByUID('legal-content', 'terms-of-use', { lang: lang });
  console.log(singleType.data);

  const folderPath = path.join(projectRoot, 'src')
  const filePath = path.join(folderPath, `/app/component/block/c37-legal-content/data/${lang === 'ru' ? '' : 'kz-'}terms-of-use.yaml`); // change as needed
