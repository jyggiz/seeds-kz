...
module.exports = async function writePrivacyPolicy(lang) {
  const singleType = await client.getByUID('legal-content', 'privacy-policy', { lang: lang });
  console.log(singleType.data);

  const folderPath = path.join(projectRoot, 'src')
  const filePath = path.join(folderPath, `/app/component/block/c37-legal-content/data/${lang === 'ru' ? '' : 'kz-'}privacy-policy.yaml`); // change as needed
