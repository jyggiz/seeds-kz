...
module.exports = async function writeFiles2(lang) {
  const singleType = await client.getByUID('files', 'files-2', { lang: lang });
  console.log(singleType.data);

  const folderPath = path.join(projectRoot, 'src')
  const filePath = path.join(folderPath, `/app/component/block/c75-image-grid/data/files-2${lang === 'ru' ? '' : '-kz'}.yaml`); // change as needed
