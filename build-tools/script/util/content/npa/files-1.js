...
module.exports = async function writeFiles1(lang) {
  const singleType = await client.getByUID('files', 'files-1', { lang: lang });
  console.log(singleType.data);

  const folderPath = path.join(projectRoot, 'src')
  const filePath = path.join(folderPath, `/app/component/block/c75-image-grid/data/files${lang === 'ru' ? '' : '-kz'}.yaml`); // change as needed
