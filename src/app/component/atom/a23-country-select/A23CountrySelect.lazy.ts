...
import './a23-country-select.scss';
...
export default class A23CountrySelect extends AbstractComponent {
  ...
  private readonly placeholder = this.valueElement?.textContent;
  ...
  private addEventListeners(): void {
    ...
    this.SetDefaultItem(this.optionElements[0]);
  ...
  private SetDefaultItem(item: HTMLElement): void {
    const { code, value, label } = item.dataset;
    if (!code || !value || !label || !this.valueElement) return;
