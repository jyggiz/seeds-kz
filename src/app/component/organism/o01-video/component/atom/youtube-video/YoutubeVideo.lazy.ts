...
export default class YoutubeVideo extends AbstractVideo {
  ...
  public async adopted() {
    ...
    this.youTubePlayer.on('stateChange', this.onStateChange.bind(this));
    this.youTubePlayer.on('ready', this.onReady.bind(this));
   ...
   * 5 - Video cued
   */
  private onStateChange(event: CustomEvent & { data: number }) {
    const eventType: string | undefined = stateToVideoEvent[event.data];

    if (eventType === VideoEvent.ENDED) {
