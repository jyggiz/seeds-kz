...
export default class O10Modal extends AbstractTransitionComponent {
  ...
  private dispatchCloseAction = async () => {
    ...
    await this.overlay?.dispatchAction({ type: MODAL.CLOSE });
  };
    ...
    switch (action.type) {
      case MODAL.STANDARD_DYNAMIC: {
        ...
        const markup = template({
          ...data,
          backButtonLabel,
          previousArrowButtonLabel,
          nextArrowButtonLabel,
        });
  ...
  public async openDynamic(markup: string): Promise<void> {
    ...
    const didLazyLoad = await lazyLoadComponents(this.modalContainer);

    if (!didLazyLoad) {
      await initComponents(this.modalContainer);
    }
  ...
  private async requestDataFragment(id: string) {
    try {
      ...
      const template = lazyModalContent[type];

      const [o11Template, overlay] = await Promise.all([template(), this.app?.overlay]);
      ...
      await overlay?.dispatchAction(action);
    } catch (err) {
      console.log(err);
    }
  }
  ...
  public async goToPreviousModal() {
    ...
    previousAction && (await overlay?.dispatchAction(previousAction));
  }
